...
int AbaloneAI::calculatePushability(const Board& board, Occupant side) {
    ...
    for (int i = 0; i < Board::NUM_CELLS; i++) {
        ...
        for (int dir = 0; dir < Board::NUM_DIRECTIONS; dir++) {
            // Check if there's a friendly marble behind the opponent (in the opposite direction)
            ...
            // Check if there's another friendly marble behind the first one (for 2-on-1 push)
            int behindBehindIdx = board.neighbors[behindIdx][oppositeDir];
            if (behindBehindIdx >= 0 && board.occupant[behindBehindIdx] == side) {
                // We have 2 friendly marbles in position for a push
                
                // Check if there's space in front to push to
                int frontIdx = board.neighbors[i][dir];
                if (frontIdx >= 0 && board.occupant[frontIdx] == Occupant::EMPTY) {
                    // This is a clean 2-on-1 push opportunity
                    pushableOpponents++;
                    continue;
                } else if (frontIdx < 0) {
                    // Can push opponent off the board
                    pushableOpponents += 2; // Worth more points!
                    continue;
                }
                
                // Check if there's an opponent marble in front that can be pushed (3-on-2)
                if (frontIdx >= 0 && board.occupant[frontIdx] == opponentSide) {
                    ...
                    if (frontFrontIdx >= 0 && board.occupant[frontFrontIdx] == Occupant::EMPTY) {
                        // This is a 2-on-2 push opportunity
                        pushableOpponents++;
                        continue;
                    } else if (frontFrontIdx < 0) {
                        // Can push two opponent marbles, one off the board
                        pushableOpponents += 3; // Worth even more!
                        continue;
                    }
                }
            }
            
            // Check for 3-on-1 push opportunities
            int behindBehindIdx = board.neighbors[behindIdx][oppositeDir];
            int behindBehindBehindIdx = -1;
            if (behindBehindIdx >= 0) {
                behindBehindBehindIdx = board.neighbors[behindBehindIdx][oppositeDir];
            }
            
            if (behindBehindIdx >= 0 && behindBehindBehindIdx >= 0 && 
                board.occupant[behindBehindIdx] == side && 
                board.occupant[behindBehindBehindIdx] == side) {
                // We have 3 friendly marbles in position for a push
                
                // Check for space or pushable opponents
                int frontIdx = board.neighbors[i][dir];
                if (frontIdx < 0 || board.occupant[frontIdx] == Occupant::EMPTY) {
                    // Can push one opponent
                    pushableOpponents += 2;
                    continue;
                } else if (board.occupant[frontIdx] == opponentSide) {
                    int frontFrontIdx = board.neighbors[frontIdx][dir];
                    if (frontFrontIdx < 0 || board.occupant[frontFrontIdx] == Occupant::EMPTY) {
                        // Can push two opponents
                        pushableOpponents += 3;
                        continue;
                    }
                }
